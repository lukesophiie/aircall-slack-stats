...
def fetch_calls(from_unix: int, page: int, per_page: int = 50) -> dict:
    ...
    params = {"from": str(from_unix), "page": page, "per_page": per_page, "order": "asc"}
...
def main():
    ...
    leaderboard = sorted(SDRS, key=lambda u: stats[u["id"]]["talk_s_total"], reverse=True)
    medals = {0: "🥇", 1: "🥈", 2: "🥉"}
    ...
    lines = []
    lines.append(f"Aircall Sales Stats · {date_str} · up to {upto_str} (Brisbane) · {SCRIPT_VERSION}")
    ...
    for i, u in enumerate(leaderboard):
        ...
        total_calls = out_total + in_total
        lines.append(f"{medal} {u['name']}: Talk {talk_m}m | Out {out_total} | In {in_total} | Total {total_calls}")
